...
#if defined(VGO_nto)
   size_t real_size;
   ret = _mmap2(addr, size, prot, flags, -1, 0, 0, 0, 0, &real_size);
   if (ret == (char *)-1) {
	   perror("mmap failed");
	   exit(1);
   }
#define ROUNDUP(a, N)   ((a + N - 1) & ~(N-1))
   if (real_size != ROUNDUP(real_size, pgsz))
		fprintf(stderr, "real_size %d incorrect (expected %d)\n", (int)real_size, (int)ROUNDUP(real_size, pgsz));
#else
