...
#define CLEAR_CALLER_SAVED_REGS  \
  do { \
   __asm__ __volatile__( "mov r0, #0" : : :/*trash*/"r0" ); \
   __asm__ __volatile__( "mov r1, #0" : : :/*trash*/"r1" ); \
   __asm__ __volatile__( "mov r2, #0" : : :/*trash*/"r2" ); \
   __asm__ __volatile__( "mov r3, #0" : : :/*trash*/"r3" ); \
  } while (0)
#else
